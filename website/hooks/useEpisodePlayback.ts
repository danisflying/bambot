...
export function useEpisodePlayback() {
  ...
  const buildImageUrl = useCallback(
    (task: string, episodeId: number, imageRef: string): string => {
      ...
      const filename = imageRef.replace(/^images\//, "");
      return `/api/episodes/${encodeURIComponent(task)}/${episodeId}/images/${encodeURIComponent(filename)}`;
    },
    []
  );
    ...
    async (summary: EpisodeSummary) => {
      ...
      try {
        const res = await fetch(
          `/api/episodes/${encodeURIComponent(summary.task)}/${summary.episode_id}`
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);

        const data: StoredEpisode = await res.json();
