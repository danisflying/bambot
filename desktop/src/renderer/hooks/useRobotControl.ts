 ...
 *
 * Phase 1: When running inside Electron (window.electron is defined), calls to
 * connectRobot() use ElectronPortHandler as the serial transport instead of the
 * browser Web Serial API, enabling native serialport access with lower latency.
 */
...
export function useRobotControl(
  ...
  // When omitted in Electron the ElectronPortHandler auto-selects the first
  // available port.  In a browser context portPath is ignored and the Web
  // Serial picker is used instead.
  const connectRobot = useCallback(async (portPath?: string) => {
    try {
      const isElectron = typeof window !== "undefined" && !!window.electron;
      if (isElectron) {
        await scsServoSDK.connect({
          portHandlerFactory: () => new ElectronPortHandler({ portPath }),
        });
      } else {
        await scsServoSDK.connect();
      }
