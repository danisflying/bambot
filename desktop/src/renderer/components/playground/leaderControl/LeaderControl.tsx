...
interface LeaderControlProps {
  leaderControl: {
    isConnected: boolean;
    connectLeader: (portPath?: string) => Promise<void>;
            ...
            <div className="flex justify-between items-center gap-2">
              {isConnected ? (
                <button
                  ...
                  disabled={connectionStatus !== "idle"}
                >
                  {connectionStatus === "disconnecting" ? "Disconnecting..." : "Disconnect Leader Robot"}
                </button>
              ) : (
                <button
                  className={`bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded flex-1 ${
                    connectionStatus !== "idle" || (isElectron && !selectedPort) ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                  onClick={handleConnect}
                  disabled={connectionStatus !== "idle" || (isElectron && !selectedPort)}
                >
                  {connectionStatus === "connecting" ? "Connecting..." : "Connect Leader Robot"}
                </button>
              )}
